...
namespace LibCoAPNonIP.Network.iOS.V1 {
    ...
    public class PeersNetwork : AbstractNetwork {
        ...
        public void AddClient(Device peer) {
            if (rr_activeClients != null) {
                rr_oplock_clients.AcquireWriterLock(-1);
                rr_activeClients.Add(peer.DisplayName, peer);
                rr_oplock_clients.ReleaseWriterLock();
            }
        }

        public void AddServer(Device peer) {
            if (rr_activeServers != null) {
                rr_oplock_servers.AcquireWriterLock(-1);
                rr_activeServers.Add(peer.DisplayName, peer);
                rr_oplock_servers.ReleaseWriterLock();
            }
        }

        public void RemoveClient(Device peer) {
            if (rr_activeClients != null) {
                rr_oplock_clients.AcquireWriterLock(-1);
                rr_activeClients.Remove(peer.DisplayName);
                rr_oplock_clients.ReleaseWriterLock();
            }
        }

        public void RemoveServer(Device peer) {
            if (rr_activeServers != null) {
                rr_oplock_servers.AcquireWriterLock(-1);
                rr_activeServers.Remove(peer.DisplayName);
                rr_oplock_servers.ReleaseWriterLock();
            }
        }

        public void RemovePeer(Device peer) {
            this.RemoveClient(peer);
            this.RemoveServer(peer);
            this.ActivePeers.Remove(peer.DisplayName);
        }
...
//            throw new NotImplementedException();
            stream.Schedule(NSRunLoop.Current , NSRunLoopMode.Default.ToString());
